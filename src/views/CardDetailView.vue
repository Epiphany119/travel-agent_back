...
const actionEditLabel = computed(() => {
  ...
  return isSocial.value && !ownsSocial.value ? '编辑为我的笔记' : '编辑笔记'
})
...
watch(() => tab.value?.id, () => {
  ...
  editing.value = tab.value.kind === 'explore-note'
}, { immediate: true })
...
async function saveNote(showMessage = true) {
  ...
  try {
    ...
    if (socialNoteId.value && ownsSocial.value) {
      const updated = await updateSocialNote(socialNoteId.value, { title: title.value, content: content.value, coverUrl: coverUrl.value, destination: destination.value, tags: tagList.value, authorName: userStore.nickname || '旅行者' })
...
async function publishToCircle() {
  ...
  try {
    ...
    if (socialNoteId.value && ownsSocial.value) {
      ...
      const response = await publishSocialNote({ userId: getCurrentUserId(), title: title.value, content: content.value, coverUrl: coverUrl.value, destination: destination.value, tags: tagList.value, authorName: userStore.nickname || '旅行者' })
      const id = Number(response.data?.id || 0)
      if (id) { socialNoteId.value = id; setTabData({ socialNoteId: id }) }
    }
...
async function copyToNotes() {
  ...
  try {
    let copied: any = null
    if (socialNoteId.value) copied = (await copySocialNote(socialNoteId.value)).data
...
function commentUser(comment: SocialComment) { return comment.user_id || '旅行者' }
</script>
  ...
  <section class="card-detail-page">
    <header class="detail-topbar">
      ...
      <div class="topbar-actions" v-if="tab">
        ...
        <button class="edit-btn" :class="{ active: editing }" :disabled="saving" @click="toggleEdit">{{ actionEditLabel }}</button>
        <button class="publish-btn" :disabled="publishing || saving" @click="publishToCircle">{{ publishing ? '发布中…' : published ? '再次发布' : '发布到圈子' }}</button>
      </div>
    </header>
    ...
    <div v-else-if="tab" class="detail-grid">
      <article class="story-column">
        <header class="story-head">
          ...
          <h1 v-else>{{ title }}</h1>
          <div class="story-meta"><span v-if="destination">⌖ {{ destination }}</span><span v-if="socialNote?.created_at">· {{ formatTime(socialNote.created_at) }}</span><span v-if="socialNote" class="status-pill"><i></i>{{ ownsSocial ? '我的圈子' : '社区公开' }}</span></div>
        </header>
...
.topbar-left, .topbar-actions, .footer-actions, .story-meta, .migration-flow, .comments-head { display: flex; align-items: center; }.topbar-left { gap: 10px; min-width: 0; }.back-btn, .quiet-btn, .edit-btn, .publish-btn, .cancel-btn, .save-main-btn, .reaction-btn { border: 1px solid var(--line); border-radius: 9px; cursor: pointer; font-weight: 700; transition: .18s ease; }.back-btn { border: 0; padding: 7px 10px; background: var(--roam-soft); color: var(--forest); }.back-btn:hover, .quiet-btn:hover, .reaction-btn:hover { transform: translateY(-1px); border-color: var(--forest); }.crumb-divider { color: var(--ink-3); }.crumb { color: var(--ink-2); font-size: 11px; letter-spacing: .12em; font-weight: 800; white-space: nowrap; }.dirty-dot { width: 7px; height: 7px; border-radius: 50%; background: var(--sunset); box-shadow: 0 0 0 4px var(--sunset-soft); }.topbar-actions { gap: 7px; flex-wrap: wrap; justify-content: flex-end; }.quiet-btn { padding: 8px 10px; background: transparent; color: var(--ink-2); font-size: 11px; }.edit-btn { padding: 8px 12px; background: var(--card); color: var(--forest); font-size: 11px; }.edit-btn.active { background: var(--roam-soft); }.publish-btn { padding: 9px 13px; background: var(--sunset); color: #fff; border-color: var(--sunset); font-size: 11px; }.publish-btn:hover:not(:disabled), .save-main-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 7px 16px color-mix(in srgb, var(--sunset) 22%, transparent); }.publish-btn:disabled, .edit-btn:disabled, .save-main-btn:disabled { opacity: .55; cursor: wait; }
