...
import { generateTravelPlan, type TravelPlan } from '@/api/agent'
...
<template>
  <main class="app-shell">
    ...
    <section v-if="plan" class="result">
      <div class="result-head"><div><p class="eyebrow">YOUR ITINERARY</p><h2>{{ plan.destination }} · {{ plan.days }} 天旅程</h2><p>{{ plan.overview }}</p></div><div class="budget"><span>预计花费</span><b>¥{{ plan.estimatedCost.toLocaleString() }}</b><small>{{ plan.budgetStatus }}</small></div></div>
      <div class="days"><button v-for="day in plan.dayPlans" :key="day.dayNumber" :class="{ active: activeDay === day.dayNumber }" @click="activeDay = day.dayNumber"><small>DAY</small><b>{{ String(day.dayNumber).padStart(2, '0') }}</b></button></div>
      <article v-if="activePlan" class="day-card"><div class="day-title"><div><p>第 {{ activePlan.dayNumber }} 天</p><h3>{{ activePlan.theme }}</h3></div><b>预算 ¥{{ activePlan.dayBudget }}</b></div><div class="timeline"><div v-for="(spot, index) in activePlan.attractions" :key="spot.name" class="timeline-item"><span>{{ index + 1 }}</span><div><h4>{{ spot.name }}</h4><p>{{ spot.description }}</p><small>建议停留 {{ spot.duration }} 小时</small></div></div></div><div class="meal-grid"><div v-for="meal in activePlan.meals" :key="meal.mealType" class="meal"><span>{{ meal.mealType }}</span><h4>{{ meal.restaurantName }}</h4><p>{{ meal.cuisine }} · 人均约 ¥{{ meal.avgPrice }}</p><small>{{ meal.reason }}</small></div></div><p class="notice">⌁ {{ activePlan.transportation }}　{{ activePlan.notes }}</p></article>
      <div class="tips"><div><h3>出发前，记住这些</h3><p v-for="tip in plan.travelTips" :key="tip">{{ tip }}</p></div><div><h3>行李清单</h3><div class="packing"><span v-for="item in plan.packingList" :key="item">✓ {{ item }}</span></div></div></div>
    </section>
  </main>
</template>
